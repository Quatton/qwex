...
# Hash: 0x3f68bdf7cf6faf77
steps:log:_timestamp() {
  date '+%Y-%m-%d %H:%M:%S'

}

# Hash: 0x93a4ed9539ccbe9e
steps:log:debug() {
  if steps:log:_should_log 2; then
  printf '%s [38;2;128;128;128m[DEBUG][38;2;255;255;255m %s\n' "$(steps:log:_timestamp)" "$1" >&2
fi

}

# Hash: 0x3b1f97c1739882ea
steps:step() {
  local description=${1:-""}
shift
local command=${*:-""}

printf '%s\n' "[38;2;128;128;128m┌──[38;2;255;255;255m [38;2;0;0;255mStep:[38;2;255;255;255m $description" >&2
...
if [ $exit_code -eq 0 ]; then
  printf '%s\n' "[38;2;128;128;128m└─[38;2;255;255;255m [38;2;0;128;0m✔ Success[38;2;255;255;255m [38;2;128;128;128m(${duration}s)[38;2;255;255;255m" >&2
else
  printf '%s\n' "[38;2;128;128;128m└─[38;2;255;255;255m [38;2;255;0;0m✘ Failed[38;2;255;255;255m [38;2;128;128;128m(Exit Code: ${exit_code})[38;2;255;255;255m" >&2
  return $exit_code
fi

}

# Hash: 0x37f79ac93b2eb751
steps:log:error() {
...
# Test uv workspace creation and execution with steps.compose
# Hash: 0xd611f6aeb7263fd1
test() {
  local failed=false
local failed_desc=""
...
if [ "$failed" = false ] || [ "false" = "true" ]; then
  if ! steps:step "Create temp directory " "mkdir -p ./uv-workspace-demo";
...
# Clean up temp directory created during test
# Hash: 0xadea0d88d8720924
clean() {
...
# Step: All done
if [ "$failed" = false ] || [ "false" = "true" ]; then
  if ! steps:step "All done " "log:info 'UV workspace test completed.'";
...
cd "$ORIGINAL_PWD"
# Step: Intentional failure step
if [ "$failed" = false ] || [ "false" = "true" ]; then
  if ! steps:step "Intentional failure step " "steps:log:error 'This is a simulated failure.'; return 1";
...
if [ "$failed" = false ] || [ "true" = "true" ]; then
  if ! steps:step "Final cleanup (always)" "clean";
...
@main() {
  case "${1:-}" in
    ...
    "all")
      ...
      ;;
    *)
      echo "Unknown task: $@. Executing as a script." >&2
      eval "$@"
      ;;
  esac
}

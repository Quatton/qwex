...
- Builtin module resolution (std/, shell/ auto-resolve to qwl/builtins/)
"""
...
class Resolver:
    ...
    def _build_module_env(
        self, module: Module, alias: str, resolved_vars: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _resolve_module_source(self, source: str, module_dir: Path) -> Path:
        ...
        """
        # Normalize source - add .yaml extension if not present
    ...
    def _load_modules_recursive(
        ...
        for mod_ref in module.modules.values():
            ...
            # Resolve source path with builtin fallback
            source_path = self._resolve_module_source(mod_ref.source, module_dir)
